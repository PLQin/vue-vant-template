...
### \# 移动端适配
本模版默认使用`px`转`vw`来进行移动端适配，已配置完成。如果需要将`px`转换为`rem`,请参考`docs`目录下面的文档说明。

### \# 路由缓存
本模版不支持 *(也不计划支持)* 全局的配置路由缓存，有需要的话可以尝试引入`vue-navigation`之类的modules或者在 `<router-view />` 处进行自定义配置。
...
### \# 日期工具类
本模版食用 [dayjs](https://github.com/xx45/dayjs)，它在GitHub上有28.2K的赞是一个使用范围极广的时间日期库，更重要的时：
- 支持
- 支持国际化
...
使用本模版建议使用`yarn`进行安装启动，安装`yarn`请执行 `npm install yarn -g`
